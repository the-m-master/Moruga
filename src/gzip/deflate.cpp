...
#define RSYNC_SUM_MATCH(sum) ((sum) % RSYNC_WIN == 0)
  ...
  static uint32_t window_size{WSIZE + WSIZE};
  ...
  static uint32_t ins_h{0}; /* hash index of string to be inserted */
...
#define H_SHIFT ((HASH_BITS + MIN_MATCH - 1) / MIN_MATCH)
  ...
  static uint32_t prev_length{0};
  /* Length of the best match at previous step. Matches not greater than this
   * are discarded. This is used in the lazy match evaluation.
   */

  uint32_t strstart{0};           /* start of string to insert */
  static uint32_t match_start{0}; /* start of matching string */
  static int32_t eofile{0};       /* flag set at end of input file */
  static uint32_t lookahead{0};   /* number of valid bytes ahead in window */
  ...
  static uint32_t max_lazy_match{0};
  /* Attempt to find a better match only when the current match is strictly
   * smaller than this value. This mechanism is used only for compression
   * levels >= 4.
   */

  static uint32_t good_match{0};
  /* Use a faster search when the previous match is longer than this */

  static uint32_t rsync_sum{0};       /* rolling sum of rsync window */
  static uint32_t rsync_chunk_end{0}; /* next rsync sequence point */
...
#define INSERT_STRING(s, match_head)               \
   ...
   */
  static void fill_window() noexcept {
   ...
   */
  static void lm_init(uint32_t pack_level) noexcept {
    ...
    lookahead = file_read(&window[0], sizeof(int32_t) <= 2 ? WSIZE : (WSIZE + WSIZE));

    if (lookahead == 0 || lookahead == uint32_t(EOF)) {
      eofile = 1;
      lookahead = 0;
      return;
    }
   ...
   * if desired.
   */
  static auto longest_match(uint32_t cur_match) noexcept -> int32_t {
...
#  error Code too clever
   ...
   */
  static void check_match(uint32_t start, uint32_t match, int32_t length) noexcept {
...
#  define check_match(start, match, length)
  ...
  /* With an initial offset of START, advance rsync's rolling checksum
     by NUM bytes.  */
  static void rsync_roll(uint32_t start, uint32_t num) noexcept {
    if (start < RSYNC_WIN) {
      /* before window fills. */
 ...
 */
#define FLUSH_BLOCK(eof) flush_block(block_start >= 0L ? reinterpret_cast<char*>(&window[uint32_t(block_start)]) : nullptr, int32_t(strstart) - block_start, flush - 1, (eof))
   ...
   */
  static auto deflate_fast() noexcept -> int32_t {
