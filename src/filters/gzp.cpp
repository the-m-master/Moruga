...
auto GZP_filter::Handle_GZ_flags(int32_t ch) noexcept -> bool {
  ...
  // ------------------------------------------------------------------------
  if (_di.flags & (1 << 2)) {
    _extra_field_length = (_extra_field_length * 10u) + uint32_t(ch);
    if (1 == _state) {
      _state = 0;
      _di.flags = uint8_t(_di.flags & ~(1u << 2));
    } else {
      _state = 1;
    }
    return false;
  ...
  if (_di.flags & (1 << 3)) {
    if (0 == ch) {
      _di.flags = uint8_t(_di.flags & ~(1u << 3));
    } else {
      return false;
    }
  }
  ...
  if (_di.flags & (1 << 4)) {
    if (0 == ch) {
      _di.flags = uint8_t(_di.flags & ~(1u << 4));
    } else {
      return false;
    }
  }
